...
//        tabLayout.addTab(tabLayout.newTab().setText("Pie-Chart View"));
        ...
        SQLiteDatabase database = openOrCreateDatabase("unique_value.db",MODE_PRIVATE,null);
        database.execSQL("create table if not exists uniqueTable(unique_item text)");
        Cursor cursor = database.rawQuery("select * from uniqueTable",null);
        if(cursor.moveToFirst() == false){
            Date now = new java.util.Date();
            Timestamp current = new java.sql.Timestamp(now.getTime());
            String time_string = now.toString() + current.toString() + android.os.Build.MODEL;
            long hash = UUID.nameUUIDFromBytes(time_string.getBytes()).getMostSignificantBits();
            String sql_query = "insert into uniqueTable values("+hash+")";
